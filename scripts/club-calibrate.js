...
function measureInPage({ angle, speed, isPutt }) {
  return new Promise((resolve) => {
    ...
    gb.reset();
    let phase = "settle", settle = 0, frames = 0;
    let start = null, y0 = 0, apex = 0, carry = null, total = null, fellOff = false;
    const obs = g.scene.onAfterRenderObservable.add(() => {
      frames++;
      const p = gb.mesh.getAbsolutePosition();
      if (phase === "settle") {
        if (++settle < 18) return;
        start = { x: p.x, y: p.y, z: p.z }; y0 = p.y;
        g.wind.speed = 0;
        gb.landed = false; gb.touchedGround = false;
      ...
      if (total === null && (gb.isLanded() || (spd < 0.2 && apex < 0.3 && frames > 20)))
        total = horiz;
...
(async () => {
  ...
  const clubs = await page.evaluate(() =>
    ClubData.CLUBS.map((c) => ({ id: c.id, name: c.name, angle: c.angle, D: c.maxDistance })));
  ...
  for (const c of clubs) {
    ...
    let lo = Math.max(5, guess * 0.45), hi = guess * 1.25;
    let best = null, bestSpeed = null;
    for (let i = 0; i < 8; i++) {
      const speed = (lo + hi) / 2;
      const r = await page.evaluate(measureInPage, { angle: c.angle, speed, isPutt });
      const dist = metric(r);
      if (dist < c.D) lo = speed; else hi = speed;
      best = r; bestSpeed = speed;
    }
    // final measure at the converged speed
    const speed = (lo + hi) / 2;
    const r = await page.evaluate(measureInPage, { angle: c.angle, speed, isPutt });
    out.push({ ...c, v0: +speed.toFixed(1), carry: +r.carry.toFixed(0), total: +r.total.toFixed(0), apex: +r.apex.toFixed(0) });
    console.log(
  ...
  for (const c of out) {
    console.log(`  id ${String(c.id).padStart(2)} ${c.name.padEnd(15)} v0: ${c.v0},`);
  }
  await browser.close();
